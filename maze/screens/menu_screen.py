...
class MenuScreen(Screen):
    def __init__(self, app):
        ...
        self.text = (
            "WASD to move. Arrow keys to shoot. E to interact. R to restart. M to mute."
        )
        self.tutorial_shadow = self.tutorial_size.render(
            self.text, True, self.shadow_color
        )
    ...
    def render(self, surface, r_scale):
        ...
        background = pygame.transform.smoothscale(
            self.background, (int(width * r_scale[0]), int(height * r_scale[1]))
        )
