...
class GameScreen(Screen):
    ...
    def process_event(self, event):
        ...
        if event.type == pygame.KEYUP and event.key == pygame.K_e:
            print("passed 'if event.key == pygame.K_e' check")
            char = self.game.get_entity_of_category(CharacterEntity)
            if char.picked_up_star:
                print("passed 'if char.picked_up_star' check")
                self.game.app.set_screen(EndScreen(self.game.app))
            elif not char.picked_up_star:
                print("did not pass char.picked_up_star check")
    ...
    def render(self, surface, r_scale):
        ...
        for hp in range(self.char.health):
            while index < self.char.health:
                pygame.draw.circle(surface, (0, 0, 0), (5 * index + 20, 20), 7)
