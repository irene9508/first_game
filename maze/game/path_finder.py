...
class PathFinder:
    ...
    def find_path(self, startxy, endxy):  # params are tuple of entity position
        ...
        start = Node(
            None, (int(startxy[0] / tile_width), int(startxy[1] / tile_height))
        )
        end = Node(None, (int(endxy[0] / tile_width), int(endxy[1] / tile_height)))
                ...
                for adj_y in range(cur_y - 1, cur_y + 2):
                    if 0 <= adj_x < map_width and 0 <= adj_y < map_height:
                        ...
                        if adj_x != cur_x and adj_y != cur_y:
                            tile1 = self.map.get_tile_properties(cur_x, adj_y, 0)
                            tile2 = self.map.get_tile_properties(adj_x, cur_y, 0)
                            if "wall" in [tile1["type"], tile2["type"]]:
                                continue

                        # update some parameters and lists:
                        extra_g = sqrt(
                            abs(current.xy[0] - adj.xy[0]) ** 2
